  ...
  try {
    ...
    if (!transferResult.status) {
      ...
      const senderBalance = await contract.methods
        ...
        .call();
      const senderEthBalance = web3.utils.fromWei(
      ...
      //데이터 베이스 최신화!
      const senderDataUpdate = await models.Wallets.update(
        { token_amount: senderBalance, eth_amount: senderEthBalance },
        {
          where: {
            user_id: sender_id,
          },
      ...
      const receiverDataUpdate = await models.Wallets.update(
      ...
      return res
        ...
        .json({
          ...
          sender_eth_balance: senderEthBalance,
